...
def main():
    parser = argparse.ArgumentParser(description="Camera detection demo with emotion and demographics overlays (model-driven)")
    parser.add_argument("--headless", action="store_true", help="Run without GUI; save annotated frames to --save-dir")
    parser.add_argument("--save-dir", type=str, default=str(repo_root / "output" / "camera_headless"), help="Directory to save frames in headless mode")
    parser.add_argument("--frames", type=int, default=-1, help="Number of frames to process in headless mode (-1 for infinite)")
