...
def build_graph(output_path: Path) -> None:
    net = Network(height="800px", width="1200px", directed=True, bgcolor="#ffffff", font_color="#222")

    # Nodes
    net.add_node("Voter", shape="icon", icon={"face": "user", "code": "f007"}, color="#8fd3f4", title="Actor: Voter")
    net.add_node("Encrypt", shape="box", color="#4db6ac", title="Fernet encryption step")
    net.add_node("Vote TX", shape="ellipse", color="#f9a825", title="Encrypted vote transaction")
